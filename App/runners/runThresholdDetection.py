"""Runner for the RT Detection (Threshold) block."""
...
_AUTOSAVE_RELPATH = os.path.join(
    "SavedTemplates", "Sorting", "RealtimeThreshold", "temp.json")
...
def _resolve_load_path(raw):
    ...
    if os.sep not in s and "/" not in s and "\\" not in s and "." not in s:
        return os.path.join(project_root(),
                            "SavedTemplates", "Sorting", "RealtimeThreshold",
                            s + ".json")
    return os.path.join(project_root(), s)


def run(inputs, params, block):
    from processing.realtime_detection_threshold import (
        load_state, save_state, _DEFAULT_PARAMS,
        RealtimeDetectionThresholdDialog,
    )
    from utils.app_logger import logger

    data = inputs.get("dataIn")
    if data is None:
        raise ValueError("RT Detection (Threshold): dataIn input is required")

    signal = np.asarray(data, dtype=float)
    if signal.ndim > 1:
        logger.info("RT Detection (Threshold): multi-channel input; "
                    "using column 0")
        signal = signal[:, 0]
    signal = signal.ravel()
    ...
    if load_path is not None and os.path.isfile(load_path):
        try:
            ...
            logger.info(
                f"RT Detection (Threshold): loaded settings from {load_path}")
        except (OSError, ValueError) as e:
            logger.warning(
                f"RT Detection (Threshold): failed to load {load_path}: {e}")

    current_path = load_path if (load_path and not auto_load) else None

    dlg = RealtimeDetectionThresholdDialog(
        signal, sr_value, detector_params,
        current_path=current_path,
    )
    if dlg.exec() != dlg.DialogCode.Accepted:
        raise ValueError("RT Detection (Threshold) cancelled")

    accepted_params, result = dlg.result_state()
    if result is None:
        raise ValueError("RT Detection (Threshold) produced no result")
    ...
    try:
        ...
        logger.warning(f"RT Detection (Threshold): auto-save failed: {e}")
